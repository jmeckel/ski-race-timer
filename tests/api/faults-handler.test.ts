...
describe('API: /api/v1/faults', () => {
  ...
  describe('POST /api/v1/faults', () => {
    ...
    it('should return 400 for invalid fault format (missing bib)', async () => {
      ...
      await handler(
        makeReq('POST', { raceId: 'test' }, { fault }),
        mockRes as any,
      );
      expect(sendBadRequest).toHaveBeenCalledWith(
        expect.anything(),
        expect.stringContaining('Invalid fault'),
      );
    });
    ...
    it('should accept valid fault and return success', async () => {
      ...
      await handler(
        makeReq(
          'POST',
          { raceId: 'test' },
          { fault, deviceId: 'dev1', deviceName: 'iPad' },
        ),
        mockRes as any,
      );
      expect(sendSuccess).toHaveBeenCalledWith(
    ...
    it('should accept fault with voice notes', async () => {
      const fault = validFault({
        notes: 'Fall at gate 5',
        notesSource: 'voice',
        notesTimestamp: new Date().toISOString(),
      });
      await handler(
        makeReq('POST', { raceId: 'test' }, { fault, deviceId: 'dev1' }),
        mockRes as any,
      );
      expect(sendSuccess).toHaveBeenCalledWith(
        expect.anything(),
  ...
  describe('DELETE /api/v1/faults', () => {
    it('should return 403 when user is not chiefJudge', async () => {
      ...
      await handler(
        makeReq('DELETE', { raceId: 'test' }, { faultId: 'fault-1' }),
        mockRes as any,
      );
      expect(sendError).toHaveBeenCalledWith(
        expect.anything(),
        'Fault deletion requires Chief Judge role',
        403,
      );
    });
    ...
    it('should delete fault when user is chiefJudge', async () => {
      ...
      await handler(
        makeReq(
          'DELETE',
          { raceId: 'test' },
          { faultId: 'fault-1', deviceId: 'dev1', approvedBy: 'Chief A' },
        ),
        mockRes as any,
      );
      expect(sendSuccess).toHaveBeenCalledWith(
    ...
    it('should return 403 when role is undefined (no role in payload)', async () => {
      vi.mocked(validateAuth).mockResolvedValueOnce({
        valid: true,
        method: 'jwt',
        payload: { type: 'race-management' } as any,
      });
      await handler(
        makeReq('DELETE', { raceId: 'test' }, { faultId: 'fault-1' }),
        mockRes as any,
