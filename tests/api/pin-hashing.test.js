...
describe('PIN Hashing (PBKDF2)', () => {
  describe('hashPin', () => {
    ...
    it('should generate hex-encoded salt and hash', () => {
      const hash = hashPin('1234');
    ...
    it('should produce unique salts for same PIN', () => {
    ...
    it('should produce different hashes for same PIN due to unique salts', () => {
      const hash1 = hashPin('1234');
      const hash2 = hashPin('1234');
  ...
  describe('verifyPin', () => {
    it('should verify correct PIN against PBKDF2 hash', () => {
      const hash = hashPin('5678');
      expect(verifyPin('5678', hash)).toBe(true);
    });

    it('should reject incorrect PIN against PBKDF2 hash', () => {
      const hash = hashPin('5678');
      expect(verifyPin('0000', hash)).toBe(false);
    });

    it('should verify all 4-digit PINs correctly', () => {
      // Test a range of PINs
      for (const pin of ['0000', '1234', '5678', '9999', '0001']) {
        const hash = hashPin(pin);
        expect(verifyPin(pin, hash)).toBe(true);
        expect(verifyPin('xxxx', hash)).toBe(false);
      }
    });

    it('should handle legacy SHA-256 hash format', () => {
    ...
    it('should return false for truncated PBKDF2 hash', () => {
      const hash = hashPin('1234');
  ...
  describe('migration path', () => {
    it('PBKDF2 hash contains colon, legacy does not', () => {
      const pbkdf2Hash = hashPin('1234');
    ...
    it('verifyPin works with both formats for same PIN', () => {
      const pin = '4567';
      const pbkdf2Hash = hashPin(pin);
      const legacyHash = crypto.createHash('sha256').update(pin).digest('hex');

      expect(verifyPin(pin, pbkdf2Hash)).toBe(true);
      expect(verifyPin(pin, legacyHash)).toBe(true);
    });

    it('upgraded hash should verify with verifyPin', () => {
      ...
      expect(verifyPin(pin, upgradedHash)).toBe(true);

      // Old PIN still shouldn't work
      expect(verifyPin('0000', upgradedHash)).toBe(false);
    });
  });
});
