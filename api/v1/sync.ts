...
import { isValidRaceId, isValidEntry, checkRateLimit, MAX_DEVICE_NAME_LENGTH } from '../lib/validation.js';
import { apiLogger, getRequestId } from '../lib/apiLogger.js';
import { detectCrossDeviceDuplicate } from '../lib/duplicateDetection.js';
import { updateDeviceHeartbeat, getActiveDeviceCount } from '../lib/deviceHeartbeat.js';
...
async function atomicAddEntry(client: Redis, redisKey: string, enrichedEntry: RaceEntry, sanitizedDeviceId: string): Promise<AtomicAddResult> {
  const result = await atomicUpdate<RaceData, AtomicAddResult>(
    ...
    (existing: RaceData) => {
      ...
      const crossDeviceDuplicate = detectCrossDeviceDuplicate(existing.entries, enrichedEntry, sanitizedDeviceId);
...
async function handleGet(
  ...
  if (sinceParam !== undefined) {
    ...
    if (!isNaN(sinceTs) && sinceTs > 0) {
      filteredEntries = allEntries.filter(
        (e: RaceEntry) => typeof e.syncedAt === 'number' && e.syncedAt >= sinceTs
      );
    }
  }
  ...
  if (limitParam !== undefined) {
    ...
    paginationMeta = { offset, limit, total: filteredTotal, hasMore: offset + limit < filteredTotal };
  }
...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  try {
    ...
    if (req.method === 'POST') {
      return await handlePost(req, res, client, normalizedRaceId, redisKey, log);
    }
