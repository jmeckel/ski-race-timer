...
import { syncService, photoStorage } from './services';
import { feedbackDelete, feedbackUndo } from './services';
...
import { initFaultEditModal, updateInlineFaultsList } from './features/faultEntry';
import { cleanupPinVerification, hasPendingPinVerification } from './features/raceManagement';
import { setModalContext, getModalContext, clearModalContext } from './utils/modalContext';
...
export function initModals(): void {
  ...
  if (confirmFaultDeleteBtn) {
    confirmFaultDeleteBtn.addEventListener('click', () => {
      ...
      const faultId = modal ? getModalContext<{ faultId: string }>(modal)?.faultId : undefined;
      if (faultId) {
  ...
  if (editRunSelector) {
    editRunSelector.addEventListener('click', (e) => {
      ...
      if (modal) {
        const ctx = getModalContext<{ entryId: string; entryRun: number }>(modal) || { entryId: '', entryRun: 1 };
 ...
 * Open confirm modal
 */
export function openConfirmModal(action: 'delete' | 'deleteSelected' | 'clearAll' | 'undoAdd'): void {
